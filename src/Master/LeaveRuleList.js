...
const LeaveRuleList = () => {
  ...
  const fetchLeaveRules = async () => {
    try {
      const res = await axios.get("http://localhost:5001/api/leaverules"); // replace with your endpoint
  ...
  const deleteLeaveRule = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:5001/api/leaverules/${id}`); // replace with your endpoint
          ...
          <table className="w-full table-auto border border-green-500 text-sm">
            <thead className="bg-gray-200">
              <tr>
                ...
                <th className="border border-green-500 px-2 py-1">Maximum Balance Enjoy</th>
              ...
              {leaveRules.length > 0 ? (
                leaveRules.map((rule, index) => (
                  <tr key={rule._id} className="hover:bg-gray-100 transition">
                    ...
                    <td className="border border-green-500 px-2 py-1">{rule.effectiveFrom}</td>
                    <td className="border border-green-500 px-2 py-1">{rule.effectiveTo}</td>
                    <td className="border border-green-500 px-2 py-1">
                      <div className="flex justify-center gap-4">
                        <button
                          onClick={() =>
                            navigate("/LeaveRuleMaste", { state: { leaveRule: rule } })
                          }
